...
Y1t=(_Node2**)malloc(sizeof(_Node2*));
Y2t=(_Node2**)malloc(sizeof(_Node2*));
if(T==NULL){*Y=NULL;return 0;}
M=(_Node2*)malloc(sizeof(_Node2));
if(T->lchild!=NULL) p=maxpath(T->lchild,Y1t,big,A,n);
else { Y1t[0]=NULL; p=0; }
if(T->rchild!=NULL) q=maxpath(T->rchild,Y2t,big,A,n);
else { Y2t[0]=NULL; q=0; }
Y1=Y1t[0]; Y2=Y2t[0];
M->key=T->key;
